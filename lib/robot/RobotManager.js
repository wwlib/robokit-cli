...
const IdentEnsembleSkill_1 = require("../rom/IdentEnsembleSkill");
class RobotManager {
    constructor() {
    ...
    start(skillName) {
        ...
        if (skill) {
            ...
            switch (skillName) {
                case 'faces':
                    skill = new FacesEnsembleSkill_1.default('faces', 'launchFaces');
                    ...
                    break;
                case 'ident':
                    skill = new IdentEnsembleSkill_1.default('ident', 'launchIdent');
                    robokit_rom_1.EnsembleSkillManager.Instance.addEnsembleSkill(skill);
                    this._robots.robotList.forEach((robot) => {
                        if (skill && robot.connected) {
                            skill.addHub(robot.hub);

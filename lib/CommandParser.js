...
class CommandParser extends events_1.EventEmitter {
            ...
            switch (command) {
                ...
                case 'say':
                    if (argsText) {
                        ...
                        const sayHelper = new SayHelper_1.default(this._appModel);
                        sayHelper.loop()
                            .then((sayResult) => {
                            resolve(sayResult);
                        });
    ...
    parseSetCommand(command, args) {
        ...
        switch (command) {
            case 'profile':
                ...
                break;
            case 'robot':
                var configId = args.join(' ');
                if (configId) {
                    var activeConfig = this._appModel.robotConfigs.setActiveRobotConfig(configId);
                    result = `current robot config set to: ${activeConfig.id}`;
                    this._appModel.saveConfig();
                }
                else {
                    let choices = this._appModel.robotConfigs.getRobotConfigIds();
                    choices.push(CANCEL_OPTION);
                    result = {
                        type: 'list',
                        name: 'mainInput',
        ...
        switch (command) {
            ...
            case 'robot':
                ...
                if (key) {
                    ...
                    var choices = RobotConfig_1.default.propertyKeys;
                    choices.push(CANCEL_OPTION);
                    result = {
                        type: 'list',
                        name: 'mainInput',
            ...
            switch (command) {
                ...
                case 'robot':
                    ...
                    if (configId) {
                        ...
                        const robotConfigHelper = new RobotConfigHelper_1.default(this._appModel);
    ...
    parseListCommand(command, args) {
        ...
        switch (command) {
            case 'profiles':
                ...
                break;
            case 'robots':
                this._appModel.robotConfigs.getRobotConfigIds().forEach((configId) => {
                    result += `${chalk.green(configId)}\n`;
                });
                break;
            case 'commands':
                this._appModel.romCommands.commandNamesWithKeyCodes.forEach((commandName) => {
    ...
    parseConnectCommand(robotConfigId, args) {
        return new Promise((resolve, reject) => {
    ...
    parseDisconnectCommand(robotConfigId, args) {
        return new Promise((resolve, reject) => {
